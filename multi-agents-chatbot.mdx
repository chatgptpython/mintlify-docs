...
GPT-trainer is the first and most powerful no-code AI Chat framework that implements a multi-agent architecture. Multiple AI Agents work together in a coordinated orchestration to enable advanced workflow automations.
...
![](multiagent2.PNG)AI Agent description is different from the Prompt. Description should tell the AI Supervisor what the Agent is designed to do at a high level, focusing on the type of user queries it should handle. Prompt is a set of instructions telling the AI Agent how to behave.
...
The Product Expert handles any user queries related to Example Company’s products. This includes questions regarding: - Product availability - Product specifications - Price - Shipping location - Warranty - Return policy - Disclaimers, legal notices, and warning labels
...
[​](https://guide.gpt-trainer.com/multi-agents-chatbot#debugging-the-chatbot)
...
![](multiagent3.PNG)This ensures that the chatbot will generate an answer based on what you revised every time it encounters the question in the future, assuming the same Agent was chosen to respond to the query.
...
To learn some best practices for training the chatbot, see [best practices for preparing training data](https://guide.gpt-trainer.com/best-practices)

To learn more about fine-tuning Agent intents, please check out the article [Fine-Tuning Agent Intents](https://guide.gpt-trainer.com/fine-tuning-agent-intents).
