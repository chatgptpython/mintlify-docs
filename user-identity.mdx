...
description: "User Identity Verification helps secure your chatbot by validating users through token-based authentication for each interaction."
---
...
2. **Configure for POST Requests:** Ensure that your API endpoint is configured to handle POST requests. This is crucial as the Chatwize will send token data via POST requests for verification.
...
1. **Token Submission:** Whenever a chat is rendered, Chatwize automatically sends a verification request to the provided API endpoint. This request includes the user's token.
...
## Step 4: Inserting Your Token into Chatwize Chatbot
...
Congratulations\! You have now successfully completed the setup of the User Identity Verification feature for your Chatwize Chatbot.
