...
description: "Create Chatbot using the Chatwize API by following this step-by-step guide. Learn how to initiate a chatbot through an API request."
---
...
- model (string, optional): Specify the model you want to use. In this example, we're using <code>
    gpt-3.5-turbo
  </code>
  .
...
- rate_limit (array, optional): Set the rate limit for the chatbot in messages per minute. It's an array with two values: [20, 240].
  ...
  First number: amount of messages. Min <code>
    1
  </code>
   - Max <code>
    100
  ...
  Second number: amount of seconds. Min <code>
    1
  </code>
   - Max <code>
    360
...
- temperature (float, optional): You can adjust the temperature parameter for response randomness. A value of 0 makes responses deterministic, while higher values increase randomness.
  ...
  Options are values as a float between <code>
    0
  </code>
   and <code>
    1
